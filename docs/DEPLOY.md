...
2. Copy `deploy/group_vars/vpn.example.yml` to
   ...
   target/SNI/short ID, and Hysteria obfuscation value.
...
4. Ensure the host firewall admits only TCP/443 and UDP/443 publicly. The agent
   management port remains bound to loopback; Xray gRPC and Hysteria auth have
   no host publication.
